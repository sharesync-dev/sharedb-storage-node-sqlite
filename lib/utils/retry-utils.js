...
/**
 * Check if an error should trigger a retry. Walks the `error.cause` chain
 * because newer expo-sqlite / better-sqlite3 wrap the underlying SQLite
 * error with a generic top-level message (e.g. "Calling the 'finalizeAsync'
 * function has failed") and place the real "database is locked" text on
 * `error.cause.message`.
